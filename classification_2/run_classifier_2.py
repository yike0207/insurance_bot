...
class BertCNN(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.conv = nn.Conv1d(in_channels=768, out_channels=768, kernel_size=3)
        self.linear = nn.Linear(in_features=768, out_features=num_class)
...
model = BertCNN.from_pretrained(pretrained_model_name_or_path=data_dir, cache_dir=bert_data_path)
...
for e in range(epoch_num):
    ...
    if acc > best_score:
        pickle.dump(train_embed_dic, Path('embed_bert_cnn.pkl').open('wb'))
