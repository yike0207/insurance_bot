...
from classification.model_zoo import FocalLoss
...
class BertFineTune(BertPreTrainedModel):
    def __init__(self, config):
        super(BertFineTune, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(in_features=768, out_features=num_class)
        self.apply(self.init_bert_weights)

    def forward(self, xid, x_seg, x_mask):
        _, pooled_output = self.bert(xid, x_seg, x_mask, output_all_encoded_layers=False)
...
model = BertFineTune.from_pretrained(pretrained_model_name_or_path=bert_model_path, cache_dir=bert_data_path)
model.to(device)
